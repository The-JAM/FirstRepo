...
class Cart extends React.Component {
    ...
    async handleSubmit(evt)
    {
        ...
        await axios.post("http://localhost:8080/api/purchaseOrder", data).then((res) => {
    ...
    {
        return (
            <Col  className="w-100" id="checkout-form">
                <Card className>
                    <Form className="h-25">
                        ...
                        <div>
                            Total:  $
                            {
                                this.state.cart ? this.calculateTotal() :
                                    0.00
                            }
        ...
        if (this.state.cart == null)
        {
            ...
            return(
                <Row>
                    <Col>
                        {
                            this.state.cart.map((item,i) => <CartItems items={item} key={i}  />)
                        }
                    </Col>
